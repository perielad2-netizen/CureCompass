...
export default function UpdateDetailPage() {
  ...
  useEffect(() => {
    if (!id) return;
    apiGet<UpdateDetail>(`/updates/${encodeURIComponent(id)}`)
      .then(setData)
      .catch((err) => {
  ...
  return (
    <main className="container-page max-w-3xl py-8">
      ...
      <div className="mt-2 flex flex-wrap items-start justify-between gap-3">
        <h1 className="min-w-0 flex-1 text-2xl font-semibold text-slate-900">{data.title}</h1>
      ...
      <p className="mt-4 text-sm font-medium text-slate-800">Why it matters: {data.why_it_matters}</p>
